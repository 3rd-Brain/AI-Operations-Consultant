...
// and viewing grades of an AI-Ops engagement workspace. Localhost only, no auth.
//
...
// Built-in stub templates per category. {{title}} / {{today}} are filled on create.
const TEMPLATES = {
  roles: (t) => `# ${t}\n\n**Department:** \n**Reports to:** \n**Last updated:** {{today}}\n\nThe goal of this role is to <one-sentence purpose>.\n\n## Responsibilities\n\n- \n\n## Tools & workflows\n\n- \n`,
  tools: (t) => `# ${t}\n\n**Website:** \n**API docs:** \n**Plan / tier:** <unknown — confirm>\n**Cloud or local:** \n**Last updated:** {{today}}\n\n## Purpose\n\n<what this tool is used for>\n\n## Records in this tool\n\n- **<record>** — <description>\n\n## Integrations\n\n- \n`,
  workflows: (t) => `# ${t}\n\n**Owner role:** → [<role-slug>.md](../roles/<role-slug>.md)\n**Last updated:** {{today}}\n\n## Jobs-to-be-done\n\nWhen <trigger>, <role> wants to <action>, so <outcome>.\n\n## SOP\n\n<!-- Step convention (powers the Workflow Map): **Action** — \`Role\` uses \`Tool\` — note -->\n<!-- Indent 2 spaces + a leading (condition) to branch; ⚠ marks an exception path. -->\n\n- [ ] **<Action>** — \`<Role>\` uses \`<Tool>\` — <note>\n\n## Automation / Agent Potential\n\n**Target KOODAR level:** <K | KA | KDA | KDAR | KODAR | KOODAR>\n**Lives in:** <which AI tool>\n**Approach:** <Use existing integration | Install / configure a skill | Custom workflow | Other>\n`,
...
async function walk(dir, base = '') {
  ...
  for (const e of entries) {
    if (e.name.startsWith('.git') || e.name === 'node_modules' || e.name === 'navigator') continue;
...
// Conventions per line (indent with 2 spaces to nest a branch under a decision):
//   - [ ] **Action** — `Role` uses `Tool` — note
//   - [ ] **Decision?** — `Role` uses `Tool` — note      (a step with children = a decision)
//     - [ ] **(if yes)** **Action** — `Role` uses `Tool` — note   (branch: leading (label))
//     - [ ] **(if no)** **Action** — `Role` uses `Tool` — ⚠ note  (⚠ or [exception] = exception)
function parseSteps(content) {
  ...
  for (const raw of sec[1].split('\n')) {
    ...
    if (codes.length === 0) continue;                 // skip the unfilled placeholder / prose
...
async function loadGrades() {
  try {
    return JSON.parse(await readFile(GRADES_PATH, 'utf8'));
  } catch {
    return {};
  }
}
...
async function buildGraph() {
  ...
  // Extract the data-architecture mermaid block (the system/data map).
  let systemMermaid = null;
  const da = files.find((f) => /data-architecture\.md$/.test(f));
  if (da) {
    const c = await readFile(path.join(ROOT, da), 'utf8').catch(() => '');
    const m = c.match(/```mermaid\s*([\s\S]*?)```/);
    if (m) systemMermaid = m[1].trim();
  }
